...
{
  internal class SelectCircularArcs : Button
  {
    protected override async void OnClick()
    {
      CancelableProgressorSource cps = new("Select circular arcs", "Canceled");
            ...
            using (RowCursor rowCursor = featLyr.Search(pSpatQuFilt))
            {
              while (rowCursor.MoveNext())
              {
                using (Row row = rowCursor.Current)
                {
                  ...
                  {
                    if (seg.SegmentType == SegmentType.EllipticArc) //collect all the circular arcs
                    {
                      if ((seg as EllipticArcSegment).IsCircular)
                      {
                        oIDs.Add(row.GetObjectID());
                        cps.Progressor.Value += 1;
                        break; //in case there are multiple segments that are circular arcs on the same feature, we only need the first.
                      }
                    }
                ...
                cps.Progressor.Status = "Features with circular arcs: " + cps.Progressor.Value;
              }
            }

            var pQuFilter = new QueryFilter();
            pQuFilter.ObjectIDs = oIDs as IReadOnlyList<long>;
            if (oIDs.Count > 0)
      ...
      if (!string.IsNullOrEmpty(errorMessage))
        MessageBox.Show(errorMessage, "Select Circular Arc Features");
      else if (!string.IsNullOrEmpty(sReportResult))
        MessageBox.Show(sReportResult, "Select Circular Arc Features");
    }
  }
}
