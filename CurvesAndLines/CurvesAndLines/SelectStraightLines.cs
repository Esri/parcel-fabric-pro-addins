...
{
  internal class SelectStraightLines : Button
  {
    protected override async void OnClick()
    {
      CancelableProgressorSource cps = new("Select straight lines", "Canceled");
            ...
            using (RowCursor rowCursor = featLyr.Search(pSpatQuFilt))
            {
              while (rowCursor.MoveNext())
              {
                using (Row row = rowCursor.Current)
                {
                  ...
                  {
                    if (seg.SegmentType == SegmentType.Line) //collect all the straight line segments
                    {
                      oIDs.Add(row.GetObjectID());
                      cps.Progressor.Value += 1;
                      break; //in case there are multiple segments that are straight line segments on the same feature, we only need the first.
                    }
                ...
                cps.Progressor.Status = "Features with straight line segments: " + cps.Progressor.Value;
              }
            }
      ...
      if (!string.IsNullOrEmpty(errorMessage))
        MessageBox.Show(errorMessage, "Select Straight Line Features");
      else if (!string.IsNullOrEmpty(sReportResult))
        MessageBox.Show(sReportResult, "Select Straight Line Features");
    }
  }
}
